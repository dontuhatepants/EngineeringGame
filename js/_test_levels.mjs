// Solvability check: brute-force every rotation combo and verify each level has at least one solution.
...
for (let li = 0; li < PIPES_LEVELS.length; li++) {
  ...
  for (let combo = 0; combo < total; combo++) {
    ...
    if (computeFlow(level, tiles)) {
      solutions++;
      // Check whether the initial state matches this solution
